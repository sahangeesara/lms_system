<x-admin-app-layout>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-neutral-500 dark:text-neutral-400 mt-0.5">
                    {{ __('edit  your curriculum offerings and status channels.') }}
                </p>
            </div>
        </div>
    ...
    <form id="courseForm" action="{{ route('admin.courses.update') }}" method="POST" enctype="multipart/form-data" class="flex-1 overflow-y-auto p-6 space-y-4 m-0">
        @csrf
        ...
        <div>
            <label class="block text-xs font-bold text-neutral-600 uppercase tracking-wider mb-1.5">{{ __('Assigned Instructor Profile') }}</label>
            <select id="course_instructor" name="instructor_id" required class="w-full text-sm bg-neutral-50 border border-neutral-200 rounded-xl px-3.5 py-2 text-black focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500">
                <option value="" disabled selected class="text-neutral-400">{{ __('Select an instructor profile...') }}</option>
                <option value="{{ Auth::id() }}">{{ __('Assign to Me') }} ({{ Auth::user()->name ?? 'Current User' }})</option>
                <option value="2">Pasindu Developer</option>
                <option value="3">Geeth Architect</option>
            </select>
        </div>

<x-instructor-app-layout>
    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-neutral-870 p-6 rounded-xl border border-neutral-900 shadow-md">
            ...
            <div>
                <a href="{{ route('instructor.lessons.create') }}" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm transition-all cursor-pointer">
                    ...
                    <tr class="hover:bg-neutral-50/40 dark:hover:bg-[#141414] transition-colors">
                        ...
                        <td class="py-4 px-6 text-right">
                            <div class="flex items-center justify-end gap-2.5">
                                ...
                                <form action="{{ route('instructor.lessons.destroy', $lesson->id) }}" method="POST" class="inline m-0" onsubmit="return confirm('{{ __('Confirm complete structural deletion of this database record row?') }}');">
                                    @csrf
                                    @method('DELETE')

<x-instructor-app-layout>
    ...
    <div class="bg-white min-h-screen p-6">
        <form id="lessonForm" action="{{ route('instructor.lessons.update', $lesson->id) }}" method="POST"
            ...
            <div>
                ...
                @php
                    // Prepare the value first to keep the HTML clean
                    $contentValue = old('content', is_array($lesson->content) ? ($lesson->content['body'] ?? '') : (json_decode($lesson->content, true)['body'] ?? $lesson->content));
                @endphp

                <textarea id="lesson_content" name="content" rows="6"
                          placeholder="Compose rich text guidelines, instructional guidelines..."
                          class="w-full px-3.5 py-2.5 text-sm bg-white border border-neutral-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-neutral-900 transition-all resize-none">{{ trim($contentValue) }}</textarea>
            </div>

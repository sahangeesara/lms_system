<x-instructor-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <button type="button" onclick="window.location.href='{{ route('instructor.courses.create') }}'" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors shadow-sm shadow-indigo-100 dark:shadow-none cursor-pointer">
                    ...
                    <tr class="hover:bg-neutral-50/40 dark:hover:bg-[#141414] transition-colors">
                        ...
                        <td class="py-4 px-6 text-right">
                            <div class="flex items-center justify-end gap-3">
                                @if(is_object($course))
                                    <button type="button"
                                            onclick="window.location.href='{{ route('instructor.courses.edit', $course) }}'"
                                    ...
                                    <form action="{{ route('instructor.courses.destroy', $course) }}" method="POST" onsubmit="return confirm('{{ __('Are you sure you want to delete this course?') }}');" class="inline m-0">
                                        @csrf
                                        @method('DELETE')
